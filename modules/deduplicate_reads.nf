process DEDUPLICATE_READS {
    ...
    output:
        tuple val(meta), path("${bam.baseName}.dedup.bam")

    script:
    // samtools -r markdup will remove duplicate reads entirely
    if(meta.sequencing_tech == "MIPS") 
        """
        samtools sort -n ${bam} | samtools fixmate -mr - ${bam.baseName}.fixed.bam
        samtools sort ${bam.baseName}.fixed.bam | samtools markdup --barcode-name - ${bam.baseName}.dedup.bam
        """

    // requires testing
    else if(meta.sequencing_tech == "hybrid-capture")
