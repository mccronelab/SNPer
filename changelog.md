...
- Fix `consensus_coverage_cutoff` failing the run when passed on the command line. `--consensus_coverage_cutoff 0.75` arrives as a String, which Groovy refuses to compare against the parsed coverage Float. Values from `nextflow.config`, a profile or a `-params-file` were already numbers and unaffected.
...
- Replace `concat` with `mix` where the amplicon, umi and positional paths rejoin in `build_consensus.nf`, resolving a workflow bottleneck caused by concat waiting for all samples before executing.
- Fix `read_deduplication` of `umi` or `positional` failing the run whenever `primer_id` is anything but `None`.
...
- Add `--help`, which prints every parameter with its type, default and description from the schema; `--help <name>` prints one parameter's full entry.
- Move to `nf-schema` 2.8.0, and set each profile's container through a `withName: '.*'` selector rather than a global `process.container`, which the plugin's run summary requires. **`-process.container` no longer overrides the image, because a selector outranks a global assignment. Use the new `container_image` param instead, or a `-c` config that sets the container through its own selector.** Every process still runs the same image.
