...
- Implement hybrid-capture read deduplication in `deduplicate_reads.nf`. The branch previously raised `error "Hybrid Capture deduplication not yet implemented."`, so any sample with `sequencing_tech: hybrid-capture` aborted the run. It now mirrors the MIPS branch (`samtools collate | fixmate -m | sort | markdup -r`), writes `${bam.baseName}.dedup.bam` to match the declared output, and records per-replicate markdup stats. First exercised by the multi-segment run; amplicon paths never reach this branch.
