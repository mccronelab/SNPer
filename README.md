...
- `primer_csv`: Path to CSV mapping `primer_id` values to primer BED files, with columns `primer_bedfile` and `primer_id`. Each sample's `primer_id` in the sample sheet selects its BED file from this table, so different samples in one run may use different primer schemes. The `primer_id` must match a row exactly, including case — a value with no matching row silently drops those samples from the run. Primer BED chrom values must match the reference FASTA headers. The `None` row is a placeholder for samples needing no primer trimming; its BED path is never read unless `read_deduplication` is `amplicon`.
...
- `min_mapped_reads`: Replicates with this many or fewer mapped reads are dropped from the run (default 1000). Counted after primer clipping and deduplication, so it reflects the reads that reach consensus calling. Each dropped replicate is named in a warning and listed in `dropped_samples.tsv` in the output directory. Because `amplicon` clipping discards every read that overlaps no primer, a replicate can align well and still fall below this — and a primer scheme that does not match the samples will drop them all. Set `--remove_unclipped_reads false` to instead retain those reads.
...
- `primer_id_default`: Default setting for sample sheet processing, useful if your sample sheet doesn't have this field (and all samples use the same primers). Must match a `primer_id` in `primer_csv`, case included; `None` is the placeholder for samples that need no primer trimming.
